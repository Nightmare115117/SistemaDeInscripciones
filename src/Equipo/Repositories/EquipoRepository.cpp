...
vector<EquipoModel> EquipoRepository::findAll() const {
    ...
    result r = txn.exec("SELECT idequipo, nombre_equipo, idlider, idproblematica, estado, registrado_en::text, coalesce(revisado_en::text, '') AS revisado_en_text, coalesce(notas, '') AS notas_text FROM equipo");
...
EquipoModel EquipoRepository::findById(int id) const {
    connection conn(dbConfig.obtenerDatabaseUrl());
    nontransaction txn(conn);
    result r = txn.exec("SELECT idequipo, nombre_equipo, idlider, idproblematica, estado, registrado_en::text, coalesce(revisado_en::text, '') AS revisado_en_text, coalesce(notas, '') AS notas_text FROM equipo WHERE idequipo = $1", params{id});
...
int EquipoRepository::insert(const EquipoModel& entity) {
    connection conn(dbConfig.obtenerDatabaseUrl());
    work txn(conn);
    result r = txn.exec(R"sql(INSERT INTO equipo (nombre_equipo, idlider, idproblematica)
        VALUES ($1, $2, $3)
...
bool EquipoRepository::update(const EquipoModel& entity) {
    ...
    result r = txn.exec(R"sql(UPDATE equipo SET
        ...
        idlider = CASE WHEN $2 <> -1 THEN $2 ELSE idlider END,
        idproblematica = CASE WHEN $3 <> -1 THEN $3 ELSE idproblematica END
        WHERE idequipo = $4)sql", params{
        entity.getNombre(),
        entity.getIdLider(),
        entity.getIdProblematica(),
        entity.getId()
